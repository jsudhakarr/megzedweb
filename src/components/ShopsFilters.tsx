...
interface ShopsFiltersProps {
  filters: ShopsFiltersState;
  onChange: (next: Partial<ShopsFiltersState>) => void;
  onReset: () => void;
}

export default function ShopsFilters({ filters, onChange, onReset }: ShopsFiltersProps) {
  return (
    <div className="space-y-5">
      ...
      <div className="space-y-2">
        ...
        <label className="flex items-center gap-2 text-sm text-slate-700">
          <input
            type="checkbox"
            checked={filters.has_active_items}
            onChange={(event) => onChange({ has_active_items: event.target.checked, page: 1 })}
          />
          Has active items
        </label>
      </div>
